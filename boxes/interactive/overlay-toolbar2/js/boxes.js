...
$.fn.sensitivize = function ($condition) {
...
enterSelectMode = function (selected) {
  ...
  $overlay.fadeIn(200).find('button').sensitivize($(':checked',$content));
  //console.log($overlay.find('button'));
  //content
  ...
  $content.each(function (i) {
    $(this).find('.box').append("<div class='check'><input type='checkbox' name='boxes'></input><span></span></div>").click(function (e) {
      ...
      //do we have checked items? enable buttons
      $('button', $overlay).sensitivize($(':checked', $content));
    });
  });
  if(selected) {
